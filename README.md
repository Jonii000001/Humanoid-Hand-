...
<img width="566" height="750" alt="Screenshot 2026-07-12 235800" src="https://github.com/user-attachments/assets/c1abd9ee-7dd0-4197-9822-b82c09154aec" />
...
Day 60: System Architecture, Signal Routing, and the BOM
Today wasn't a particularly flashy day, as I mostly focused on research and planning. My main goal was figuring out how to connect all 30 magnetic encoders so the Raspberry Pi can actually read their signals without creating an absolute mess of wires. I initially used Gemini for advice and looked up some hardware multiplexers online, but they turned out to be way too expensive for my budget. After brainstorming a bit more with Claude AI, I finally landed on the perfect, cost-effective solution: using the RP2040-Zero microcontroller board to handle the sensor data.

I also mapped out the actual wiring connections. I will need to route the SCK (Serial Clock) and Data lines from top to bottom, linking the three encoders on one side of the finger and matching them with the ones on the other side.

Besides the wiring logic, I spent some time updating my Bill of Materials (BOM). I realized that I had completely forgotten to add a crucial component yesterday: a 100-ohm resistor. At first, I couldn't find the exact 0402 package 100-ohm resistor from the brand Yageo in the supplier's library, but after some digging, I finally tracked it down. Now, I'm faced with a new dilemma: how to handle the manufacturing. I need to decide whether I should solder these microscopic 0402 SMD components myself, or if I should just use JLCPCB's PCBA (Printed Circuit Board Assembly) service to have it done directly at the factory. I still need to check how much the assembly service will cost—it might be expensive, but it could save me hours of incredibly tedious and difficult soldering.
